...
describe("TimerFunction", () => {
	describe("#onTick(event)", () => {
		it("should invoke onTick(event)", async () => {
			...
			let _date = moment();
			_azcontext.bindings.tick.schedule.scheduleStatus.last = moment(_date).subtract(5, "minutes").format();
			_azcontext.bindings.tick.schedule.scheduleStatus.lastUpdated = moment(_date).subtract(4, "minutes").format();
			_azcontext.bindings.tick.schedule.scheduleStatus.next = moment(_date).add(5, "minutes").format();
